...
class Data {
  ...
  async init() {
    ...
    [
      ...
      this.user4Signer.getAddress(),
      this.user5Signer.getAddress(),
      this.user6Signer.getAddress(),
      this.user7Signer.getAddress(),
      this.user8Signer.getAddress(),
    ]);
      ...
      data.eventFragment.inputs.forEach((a) => {
        ...
        if (a.type == "address") {
          ...
          if (
            ...
            a.name == "againstVotes" ||
            a.name == "tokensBurnt" ||
            a.name == "tokensWithSlashingFactor" ||
            a.name == "rewardWithSlashingFactor" ||
            a.name == "cap"
          ) {
            ...
            result =
              result + ethers.utils.formatUnits(data.args[a.name], 16) + "%";
          } else if (a.name == "term") {
  ...
  async expectException(message, searchString, promise) {
    try {
      ...
      console.log(e);
      assert(
        e.toString().indexOf(searchString) >= 0,
        message +
          " - '" +
          searchString +
    ...
    assert.fail(
      message + " - Exception '" + searchString + "' was not thrown as expected"
    );
  }
  ...
  async printTxData(message, tx) {
    ...
    console.log(
      "        " +
        ...
        ", feeUsd: " +
        ethers.utils.formatUnits(feeUsd, 18) +
        ", @ " +
        ethers.utils.formatUnits(tx.gasPrice, 9) +
        " gwei & " +
        ethers.utils.formatUnits(this.ethUsd, 18) +
        " ETH/USD, " +
        tx.hash
    );
  ...
  async printBalances() {
    ...
    for (let i = 0; i < this.accounts.length; i++) {
      ...
      let etherBalanceBaseBlock = (
        await ethers.provider.getBalance(account, this.baseBlock)
      ).toString();
      ...
      line =
        ...
        this.padLeft(i, 2) +
        " " +
        this.padRight(this.getShortAccountName(account), 25) +
        " " +
        this.padToken(etherBalanceDiff, 18);
      for (let t = 0; t < this.tokenContracts.length; t++) {
        line =
          line +
          this.padToken(
            tokenBalances[t] || BigNumber.from(0),
            this.decimals[t] || 18
          );
      }
      console.log(line);
    ...
    for (let t = 0; t < this.tokenContracts.length; t++) {
      line =
        line +
        this.padToken(
          totalTokenBalances[t] || BigNumber.from(0),
          this.decimals[t] || 18
        );
    }
    console.log(line);
    ...
    for (let i = 0; i < this.tokenContracts.length; i++) {
      ...
      let [symbol, name, decimals, totalSupply] = await Promise.all([
      ...
      console.log(
        "        Token " +
          ...
          ", address: " +
          this.getShortAccountName(tokenContract.address) +
          " @ " +
      ...
      try {
        ...
        console.log(
          "          # Permissioned account     Role            Active                  Maximum                Processed"
        );
        console.log(
          "         -- ------------------------ --------------- ------ ------------------------ ------------------------"
        );
      ...
      if (symbol == "OptinoGov" && this.optinoGov != null) {
        let [
          ...
          maxDuration,
          collectRewardForFee,
          collectRewardForDelay,
          proposalCost,
          proposalThreshold,
        ] = await Promise.all([
          this.optinoGov.ogToken(),
          this.optinoGov.ogdToken(),
          this.optinoGov.ogRewardCurve(),
          this.optinoGov.voteWeightCurve(),
          this.optinoGov.accountsLength(),
          this.optinoGov.maxDuration(),
          this.optinoGov.collectRewardForFee(),
          this.optinoGov.collectRewardForDelay(),
          this.optinoGov.proposalCost(),
          this.optinoGov.proposalThreshold(),
        ]);
        let [
          quorum,
          quorumDecayPerSecond,
          votingDuration,
          executeDelay,
          totalVotes,
        ] = await Promise.all([
          this.optinoGov.quorum(),
          this.optinoGov.quorumDecayPerSecond(),
          this.optinoGov.votingDuration(),
          this.optinoGov.executeDelay(),
          this.optinoGov.totalVotes(),
        ]);
        let [proposalsLength /*, stakeInfoLength*/] = await Promise.all([
          this.optinoGov.proposalsLength() /*, this.optinoGov.stakeInfoLength()*/,
        ]);
        console.log(
          "        - ogToken              : " +
            this.getShortAccountName(ogToken)
        );
        console.log(
          "        - ogdToken             : " +
            this.getShortAccountName(ogdToken)
        );
        console.log(
          "        - ogRewardCurve        : " +
            this.getShortAccountName(ogRewardCurve)
        );
        console.log(
          "        - voteWeightCurve      : " +
            this.getShortAccountName(voteWeightCurve)
        );
        let decimals = 18;
        console.log(
          "        - maxDuration          : " +
            maxDuration +
            " seconds = " +
            maxDuration.div(60 * 60 * 24) +
            " days"
        );
        console.log(
          "        - collectRewardForFee  : " +
            ethers.utils.formatUnits(collectRewardForFee, 16) +
            "%"
        );
        console.log(
          "        - collectRewardForDelay: " +
            collectRewardForDelay +
            " seconds = " +
            collectRewardForDelay.div(60 * 60 * 24) +
            " days"
        );
        console.log(
          "        - proposalCost         : " +
            ethers.utils.formatUnits(proposalCost, decimals)
        );
        console.log(
          "        - proposalThreshold    : " +
            ethers.utils.formatUnits(proposalThreshold, 16) +
            "%"
        );
        console.log(
          "        - quorum               : " +
            ethers.utils.formatUnits(quorum, 16) +
            "%"
        );
        console.log(
          "        - quorumDecayPerSecond : " +
            ethers.utils.formatUnits(
              quorumDecayPerSecond.mul(60 * 60 * 24 * 365),
              16
            ) +
            "% per year"
        );
        ...
        console.log(
          "        - executeDelay         : " +
            executeDelay +
            " seconds = " +
            executeDelay.div(60 * 60 * 24) +
            " days"
        );
        console.log(
          "        - totalVotes           : " +
            ethers.utils.formatUnits(totalVotes, decimals)
        );
        // console.log("        - proposalsLength      : " + proposalsLength);
        // console.log("        - stakeInfoLength      : " + stakeInfoLength);
        console.log(
          "          # Account                Duration              End                    Rate%                  Balance                    Votes Delegatee                     Delegated Votes                  Accrued Accrual Term"
        );
        console.log(
          "         -- -------------------- ---------- ---------------- ------------------------ ------------------------ ------------------------ -------------------- ------------------------ ------------------------ ------------"
        );
        for (let j = 0; j < accountsLength; j++) {
          ...
          const accruedReward = await tokenContract.accruedReward(
            _a.tokenOwner
          );
          console.log(
            "         " +
              ...
              " " +
              this.padLeft(
                this.termString(_a.account.duration.toString()),
                10
              ) +
              " " +
              this.padLeft(
                this.termString(parseInt(_a.account.end.toString()) - now),
                16
              ) +
              " " +
              ...
              " " +
              this.padLeft(
                ethers.utils.formatUnits(_a.account.balance, 18),
                24
              ) +
              " " +
              ...
              " " +
              this.padRight(
                this.getShortAccountName(_a.account.delegatee),
                20
              ) +
              " " +
              this.padLeft(
                ethers.utils.formatUnits(_a.account.delegatedVotes, 18),
                24
              ) +
              " " +
        ...
        console.log(
          "          #      Start Executed Proposer             Description                                                                 For Votes            Against Votes"
        );
        console.log(
          "         -- ---------- -------- -------------------- ------------------------------------------------------------ ------------------------ ------------------------"
        );
        for (let j = 0; j < proposalsLength; j++) {
          ...
          console.log(
            "         " +
              ...
              " " +
              this.padLeft(
                this.termString(parseInt(proposal.start.toString()) - now),
                10
              ) +
              " " +
              this.padLeft(
                proposal.executed.toString() == 0 ? "no" : "yes",
                8
              ) +
              " " +
              ...
              " " +
              this.padLeft(
                ethers.utils.formatUnits(proposal.forVotes, 18),
                24
              ) +
              " " +
              this.padLeft(
                ethers.utils.formatUnits(proposal.againstVotes, 18),
                24
              )
          );
          for (let k = 0; k < proposal.targets.length; k++) {
            console.log(
              "           + exec " +
                k +
                ". target: " +
                this.getShortAccountName(proposal.targets[k]) +
                ", value: " +
                ethers.utils.formatUnits(proposal._values[k], 18) +
                ", data: " +
                proposal.data[k]
            );
          }
          ...
          for (let k = 1; k < 5; k++) {
            ...
            votes =
              votes +
              separator +
              this.getShortAccountName(this.accounts[k]) +
              ": " +
              voted;
            separator = ", ";
          }
        ...
        const [cap, freezeCap] = await Promise.all([
          tokenContract.cap(),
          tokenContract.freezeCap(),
        ]);
        console.log(
        ...
        const dividendTokensLength = parseInt(
          await tokenContract.dividendTokensLength()
        );
        console.log(
          "          # Dividend         Enabled                  Unclaimed"
        );
        console.log(
          "         -- ---------------- ------- --------------------------"
        );
        ...
        for (let j = 0; j < dividendTokensLength; j++) {
          ...
          const unclaimedDividends = await tokenContract.unclaimedDividends(
            dividendToken[0]
          );
          dividendHeader =
            dividendHeader +
            this.padLeft(
              "Owing " + this.getShortAccountName(dividendToken[0]),
              24
            ) +
            " " +
            this.padLeft(
              "New " + this.getShortAccountName(dividendToken[0]),
              24
            ) +
            " ";
          dividendSeparator =
          ...
          console.log(
            "         " +
              this.padLeft(j, 2) +
              " " +
              this.padRight(this.getShortAccountName(dividendToken[0]), 18) +
              "  " +
              this.padRight(dividendToken[1].toString(), 6) +
              " " +
              this.padLeft(ethers.utils.formatUnits(unclaimedDividends, 18), 24)
          );
        }
        console.log(
          "         -- ---------------- ------- --------------------------"
        ...
        if (dividendTokensLength > 0) {
          ...
          for (let j = 1; j < this.accounts.length; j++) {
            ...
            if (
              ...
              !accountName.startsWith("OG") &&
              !accountName.startsWith("Vote")
            ) {
              const dividendsOwing = await tokenContract.dividendsOwing(
                account
              );
              let result = "";
              ...
              for (let k = 0; k < dividendTokensLength; k++) {
                result =
                  result +
                  this.padLeft(ethers.utils.formatUnits(owingList[k], 18), 24) +
                  " " +
                  this.padLeft(
                    ethers.utils.formatUnits(newOwingList[k], 18),
                    24
                  ) +
                  " ";
              }
              console.log(
                "         " +
                  this.padLeft(j, 2) +
                  " " +
                  this.padRight(this.getShortAccountName(account), 18) +
                  " " +
                  result
              );
            }
        ...
        const [
          ...
          /*weightedDurationDenominator,*/ slashingFactor,
        ] = await Promise.all([
          tokenContract.getStakingInfo(),
          tokenContract.owner(),
          tokenContract.accountsLength(),
          tokenContract.weightedEnd(),
          tokenContract.weightedEndNumerator(),
          /*tokenContract.weightedDurationDenominator(),*/ tokenContract.slashingFactor(),
        ]);
        console.log(
          "        - staking @ " +
            this.getShortAccountName(tokenContract.address) +
        ...
        console.log(
          "          - dataType                   : " +
            stakingInfo.dataType.toString()
        );
        console.log(
          "          - addresses                  : " +
            JSON.stringify(
              stakingInfo.addresses.map((x) => {
                return this.getShortAccountName(x);
              })
            )
        );
        console.log(
        ...
        console.log(
          "          - weightedEnd                : " +
            this.termString(parseInt(weightedEnd.toString()) - now) +
            " " +
            weightedEnd +
            " = " +
            ethers.utils.formatUnits(weightedEndNumerator, decimals) +
            "/" +
            ethers.utils.formatUnits(totalSupply, decimals)
        );
        console.log(
          "          - slashingFactor             : " +
            ethers.utils.formatUnits(slashingFactor, 16) +
            "%"
        );
        console.log(
          "          # Account                    Duration              End      Index                    Rate%                  Balance           Accrued Reward Accrual Term"
        );
        console.log(
          "         -- -------------------- -------------- ---------------- ---------- ------------------------ ------------------------ ------------------------ ------------"
        );
        for (let k = 0; k < accountsLength; k++) {
          ...
          console.log(
            "         " +
              ...
              " " +
              this.padLeft(
                this.termString(account.account.duration.toString()),
                14
              ) +
              " " +
              this.padLeft(
                this.termString(parseInt(account.account.end.toString()) - now),
                16
              ) +
              " " +
              ...
              " " +
              this.padLeft(
                ethers.utils.formatUnits(account.account.rate, 16),
                24
              ) +
              " " +
              this.padLeft(
                ethers.utils.formatUnits(account.account.balance, 18),
                24
              ) +
              " " +
              this.padLeft(ethers.utils.formatUnits(accruedReward[0], 18), 24) +
    ...
    if (this.ogRewardCurve != null) {
      const [owner, pointsLength] = await Promise.all([
        this.ogRewardCurve.owner(),
        this.ogRewardCurve.pointsLength(),
      ]);
      console.log(
        "        SimpleCurve " +
          this.getShortAccountName(this.ogRewardCurve.address) +
          " @ " +
          this.ogRewardCurve.address +
          ", owner: " +
          this.getShortAccountName(owner) +
          ", pointsLength: " +
          pointsLength
      ...
      for (let j = 0; j < pointsLength; j++) {
        ...
        console.log(
          "         " +
            ...
            " " +
            this.padLeft(
              ethers.utils.formatUnits(point.rate.div(365 * 24 * 60 * 60), 16),
              24
            )
        );
      }
    ...
    if (this.voteWeightCurve != null) {
      const [owner, pointsLength] = await Promise.all([
        this.voteWeightCurve.owner(),
        this.voteWeightCurve.pointsLength(),
      ]);
      console.log(
        "        SimpleCurve " +
          this.getShortAccountName(this.voteWeightCurve.address) +
          " @ " +
          this.voteWeightCurve.address +
          ", owner: " +
          this.getShortAccountName(owner) +
          ", pointsLength: " +
          pointsLength
    ...
    if (this.stakingRewardCurve != null) {
      const [owner, pointsLength] = await Promise.all([
        this.stakingRewardCurve.owner(),
        this.stakingRewardCurve.pointsLength(),
      ]);
      console.log(
        "        SimpleCurve " +
          this.getShortAccountName(this.stakingRewardCurve.address) +
          " @ " +
          this.stakingRewardCurve.address +
          ", owner: " +
          this.getShortAccountName(owner) +
          ", pointsLength: " +
          pointsLength
    ...
    if (this.stakingFactory != null) {
      console.log(
        "        StakingFactory " +
          this.getShortAccountName(this.stakingFactory.address) +
          " @ " +
      ...
      const [
        ...
        this.stakingFactory.stakingTemplate(),
        this.stakingFactory.ogToken(),
        this.stakingFactory.ogdToken(),
        this.stakingFactory.stakingRewardCurve(),
        this.stakingFactory.stakingsLength(),
      ]);
      console.log(
        "        - stakingTemplate        : " +
          this.getShortAccountName(stakingTemplate)
      );
      console.log(
        "        - ogToken                : " +
          this.getShortAccountName(ogToken)
      ...
      console.log(
        "        - stakingRewardCurve     : " +
          this.getShortAccountName(stakingRewardCurve)
      );
      ...
      for (let j = 0; j < stakingsLength; j++) {
        ...
        const [
          ...
          staking.owner(),
          staking.accountsLength(),
          staking.totalSupply(),
          staking.weightedEnd(),
          staking.slashingFactor(),
        ]);
        console.log(
          "          - staking " +
            j +
            " @ " +
            this.getShortAccountName(stakingAddress[1]) +
            ", owner: " +
            this.getShortAccountName(owner)
        );
      }
  ...
  async displayAccounts(detailedBalances = false) {
    ...
    if (detailedBalances) {
      accountsTable = new Table({
        ...
        colWidths: [5, 30, 50, 30, 30, 30],
      });
    } else {
      accountsTable = new Table({
        head: ["ID", "Name", "Address", "Current Balance"],
        colWidths: [5, 30, 50, 30],
      });
    }
  ...
  async hasEvent(ethers, tx, eventNamePlusReturn) {
    const receipt = await ethers.provider.getTransactionReceipt(tx.hash);
    let eventSig = ethers.utils.keccak256(
      ethers.utils.toUtf8Bytes(eventNamePlusReturn)
    );
    return receipt.logs.filter((x) => x.topics[0] === eventSig);
  }

  async getEventArgs(ethers, tx, abi) {
    const eventNamePlusReturn = this.toEventSig(abi);
    const receipt = await ethers.provider.getTransactionReceipt(tx.hash);
    let eventSig = ethers.utils.keccak256(
      ethers.utils.toUtf8Bytes(eventNamePlusReturn)
    );
    let logItem = receipt.logs.filter((x) => x.topics[0] === eventSig)[0];

    let iface = new ethers.utils.Interface(["event " + abi]);
    return iface.parseLog(logItem).args;
  }

  // Redeem(uint16 sourceTokenId, bytes32 randomRequestHash)
  toEventSig(eventNamePlusReturn) {
    const str = eventNamePlusReturn.split("("); // uint16 sourceTokenId, bytes32 randomRequestHash)
    const parts = str[1].split(", "); // uint16 sourceTokenId / bytes32 randomRequestHash
    const types = [];
    for (let i = 0; i < parts.length; i++) {
      const bits = parts[i].split(" "); // uint16 / sourceTokenId
      types.push(bits[0]);
    }
    let retval = "";
    retval += str[0] + "(";
...
async function onlyGivenAddressCanInvoke({
  ...
  for (const user of accounts) {
    ...
    contract.connect(user);

    await assert.revert(await contract[method](...args), reason);
  }
  if (!skipPassCheck && address) {
    await method(...args, { from: address });
  }
}
